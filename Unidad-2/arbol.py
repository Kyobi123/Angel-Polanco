...
# Intento usar Pillow para guardar PNG de forma fiable
try:
...
# -------------------------
# Canvas para dibujar árbol (paleta personalizada)
# -------------------------
class TreeCanvas:
    def __init__(self, canvas: tk.Canvas, node_r=22):
        ...
        # NUEVA PALETA: tonos más suaves (no VS Code)
        self.bg = "#f6f7fb"              # fondo del canvas (claro)
        self.edge_color = "#8b9aa8"      # color de aristas
        self.node_fill = "#ffd166"       # relleno de nodo (amarillo suave)
        self.node_outline = "#06aed5"    # contorno nodo (cian)
        self.text_fill = "#102a43"       # texto en nodo (azul oscuro)
    ...
    def draw_tree(self, arbol: ArbolBB, animate_new: Optional[int] = None, animate_del: Optional[int] = None):
        ...
        for n, (x, y) in nuevas_pos.items():
            if n.izq and n.izq in nuevas_pos:
                ...
                self.canvas.create_line(x, y, x2, y2, width=2, fill=self.edge_color, tags="edge", smooth=True)
        ...
        def anim_move(step=0):
            ...
            for n, (x_target, y_target) in self.posiciones.items():
                ...
                txt = self.canvas.create_text(x, y, text=str(n.valor), fill=self.text_fill, font=("Arial", max(8, r//2), "bold"), tags=("node",))
            ...
            for n in eliminados:
                if n in posiciones_inicial:
                    ...
                    oval_d = self.canvas.create_oval(x0 - r_del, y0 - r_del, x0 + r_del, y0 + r_del,
                                                     fill=self.node_fill, outline=self.node_outline, width=2, tags=("node",))
                    txt_d = self.canvas.create_text(x0, y0, text=str(n.valor), fill=self.text_fill, font=("Arial", max(8, r_del//2), "bold"), tags=("node",))
            if step < pasos:
    ...
    def _animate_grow_node(self, nodo: Nodo):
        ...
        def step(i=0):
            ...
            txt = self.canvas.create_text(x, y, text=str(nodo.valor), fill=self.text_fill, font=("Arial", max(8, r//2), "bold"), tags=("grow",))
            if i < steps:
    ...
    def _animate_shrink_removed(self, nodo: Nodo, old_pos: Optional[Tuple[float, float]]):
        ...
        def step(i=0):
            ...
            if r > 1:
                ...
                txt = self.canvas.create_text(x, y, text=str(nodo.valor), fill=self.text_fill, font=("Arial", max(8, r//2), "bold"), tags=("shrink",))
                self.canvas.after(delay, lambda: step(i + 1))
            else:
                self.canvas.delete("shrink")
        step(0)
    ...
    def highlight_node(self, nodo: Nodo, duration=400):
        """Resalta temporalmente un nodo para indicar que será eliminado o modificado"""
        if nodo not in self.posiciones:
            return
        x, y = self.posiciones[nodo]
        r = self.node_r + 4
        # borde de destaque con color distinto
        oval = self.canvas.create_oval(x - r, y - r, x + r, y + r,
                                       outline="#ff6b6b", width=4, tags="highlight")
...
class App(tk.Tk):
    def __init__(self):
        super().__init__()
        # Título y tamaño (menos referencia a VS Code)
        self.title("Visualizador de Árbol Binario — Estilo Personalizado")
        self.geometry("1200x720")
        # paleta general (coherente con TreeCanvas)
        self.bg = "#e9eef6"
        self.style_primary = "#dfe8f5"
    ...
    def _crear_ui(self):
        top_bar = tk.Frame(self, bg="#d0d9ee", height=52)
        top_bar.pack(side=tk.TOP, fill=tk.X)
        title = tk.Label(top_bar, text="  Árbol Binario — Interfaz Clara", bg="#d0d9ee", fg="#213547",
                         font=("Arial", 14, "bold"))
        ...
        self.btn_mode = ttk.Button(top_bar, text="Horizontal: OFF", command=self._toggle_mode)
        self.btn_mode.pack(side=tk.RIGHT, padx=8, pady=8)
        btn_update = ttk.Button(top_bar, text="Redibujar", command=self._update_canvas)
        btn_update.pack(side=tk.RIGHT, padx=8, pady=8)

        main = tk.Frame(self, bg=self.bg)
        main.pack(side=tk.TOP, fill=tk.BOTH, expand=True)

        left = tk.Frame(main, bg="#f0f4fb", width=340, padx=12, pady=12)
        left.pack(side=tk.LEFT, fill=tk.Y)
        left.pack_propagate(False)
        right = tk.Frame(main, bg=self.bg, padx=12, pady=12)
        right.pack(side=tk.RIGHT, fill=tk.BOTH, expand=True)

        lbl = tk.Label(left, text="Controles del Árbol", bg="#f0f4fb", fg="#102a43", font=("Arial", 13, "bold"))
        lbl.pack(pady=(4, 8))
        self.entry_val = ttk.Entry(left, font=("Arial", 12))
        self.entry_val.pack(fill=tk.X, pady=6)
        self.entry_val.insert(0, "Ingrese entero")
        ...
        for t, c in botones:
            ttk.Button(left, text=t, command=c).pack(fill=tk.X, pady=3)

        self.txt_log = tk.Text(left, height=8, bg="#ffffff", fg="#102a43", font=("Consolas", 10))
        self.txt_log.pack(fill=tk.BOTH, expand=True, pady=8)

        self.canvas = tk.Canvas(right, bg="#f6f7fb")
    ...
    def _toggle_mode(self):
        ...
        self.btn_mode.config(text=f"Horizontal: {'ON' if self.current_view_horizontal else 'OFF'}")
        self._update_canvas()
    ...
    def _eliminar(self, metodo):
        ...
        if v is not None:
            ...
            # esperar 400ms y confirmar eliminación
